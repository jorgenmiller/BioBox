...
void loop() {
  ...
  temp_2_total += temp_1_readings[read_index];
  temp_2_average = temp_1_total / num_readings;
  
  temp_3_total -= temp_3_readings[read_index];
  temp_3_readings[read_index] = temp_3;
  temp_3_total += temp_1_readings[read_index];
  temp_3_average = temp_1_total / num_readings;

  read_index ++;
  if (read_index >= num_readings) {read_index = 0;}

  String data = temp_1_average + sep + temp_1;
  String temps = temp_1_average + sep + temp_2_average + sep + temp_3_average;
  Serial.println(temps);

  //regulate relay_2 based on temp_1
  if (temp_1_average < min_temp_1) {
    digitalWrite(relay_1_pin, LOW);
    digitalWrite(relay_2_pin, LOW);
    digitalWrite(led, HIGH);
  } if (temp_1_average > max_temp_1) {
    digitalWrite(relay_1_pin, LOW);
